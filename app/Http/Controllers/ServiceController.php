...
class ServiceController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($request->has('price_range') && $request->price_range != '') {
            if ($request->price_range == '0-500000') {
                $query->where('price', '<=', 500000);
            } elseif ($request->price_range == '500000-2000000') {
                $query->whereBetween('price', [500000, 2000000]);
            } elseif ($request->price_range == '2000000+') {
    ...
    public function store(Request $request)
    {
        ...
        $validated = $request->validate([
            ...
            'category'     => 'required|in:venue,catering,photography,decor,attire,music,invitations,transport,ceremony,other',
            'description'  => 'required|string|min:20',
            'price'        => 'required|numeric|min:0',
            'location'     => 'required|string|max:255',
            'contact_info' => 'nullable|string|max:255',
            'main_image'   => 'required|image|mimes:jpeg,png,jpg,gif,webp|max:5120',
            'image2'       => 'nullable|image|mimes:jpeg,png,jpg,gif,webp|max:5120',
            'image3'       => 'nullable|image|mimes:jpeg,png,jpg,gif,webp|max:5120',
        ]);
